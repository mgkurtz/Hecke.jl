...
@doc raw"""
    _basis_of_commutator_algebra(As, Bs) -> Vector{elem_type(As)}

Basis of the vector space of all $M$ which satisfy $MA = BM$ for all $(A, B)$ in `zip(As, Bs)`.

For `As` being $n\times n$-matrices and `Bs` being $m\times m$-matrices, the result will consist of $m\times n$-matrices.
For `As == Bs` the spanned vector space is the algebra of all $M$ with $MA = AM$ for all `A in As`.
"""
function _basis_of_commutator_algebra(As, Bs)
  K = _basis_of_commutator_algebra_as_cols(As, Bs)
  n = nrows(first(As))
  m = nrows(first(Bs))
  # res = eltype(As)[reshape(k, m, n) for k in eachcol(K)] # currently unsupported
  res = eltype(As)[]
...
function _basis_of_integral_commutator_algebra(As, Bs)
  ...
  n = nrows(first(As))
  m = nrows(first(Bs))
  # res = QQMatrix[reshape(k, m, n) for k in eachrow(K)] # currently unsupported
  res = QQMatrix[]
  for k in axes(K, 1)
    cartind = CartesianIndices((n, m))
...
function _basis_of_commutator_algebra_as_cols(As, Bs)
